...
const STAT_CARDS = [
  ...
  { key: "totalConsultations", label: "Total Consultations",   color: "bg-yellow-500", icon: "🩺" },
  { key: "totalPrescriptions", label: "Total Prescriptions",   color: "bg-pink-500",   icon: "💊" },
      ...
      <main className="max-w-7xl mx-auto px-6 py-8">
        ...
        {error && (
          <div className="bg-red-50 border border-red-200 text-red-600
            rounded-lg p-4 mb-6 flex justify-between">
            <span>{error}</span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          ...
          {/* Bill breakdown */}
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
            <h3 className="font-semibold text-gray-700 mb-4">🧾 Billing Status</h3>
            {loading ? <Skeleton rows={3} /> : (
              <div className="space-y-3">
                <StatusBar
                  label="Unpaid"
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
            ...
            <div className="space-y-2">
              {[
                ...
                { label: "Register Nurse",    path: "/nurses/register", color: "bg-purple-600" },
              ].map(({ label, path, color }) => (
                <button key={path}
                  onClick={() => navigate(path)}
                  className={`${color} text-white w-full text-left
                    px-3 py-2 rounded-lg text-sm hover:opacity-90`}>
                  {label}
                </button>
              ))}
            </div>
  ...
  return (
    <div>
      <div className="flex justify-between text-sm mb-1">
        ...
        <span className="font-semibold text-gray-800">{value}
